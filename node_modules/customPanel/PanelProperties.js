...
import * as approvalDate from './parts/ApprovalProps';
import * as dateOfDelivery from './parts/DateOfDelivery';
import * as contacts from './parts/ContactsProps';
import * as functionalityDomain from './parts/FunctionalityDomainProps';
import * as documentProperty from './parts/DocumentPropertyProps';
import * as advantages from './parts/AdvantagesProps';
import * as interval from './parts/IntervalProps';
import * as presentationChannel from './parts/PresentationChannelProps';
import * as requestCondition from './parts/RequestConditionProps';
import * as price from './parts/PriceProps';
import * as tutorial from './parts/TutorialProps';
import * as acceptingConditions from './parts/AcceptingConditionsProps';
import * as specialist from './parts/SpecialistProps';
import * as organization from './parts/OrganizationProps';
...
function createForthTabGroups(element, bpmnFactory, elementRegistry, translate) {
    ...
    price.getElement(forthGroup, element);
...
function createFifthTabGroups(element, bpmnFactory, elementRegistry, translate) {
    ...
    presentationChannel.getElement(thirdGroup, element);
    advantages.getElement(thirdGroup, element);
    interval.getElement(thirdGroup, element);
    requestCondition.getElement(thirdGroup, element);
...
function createSixthTabGroups(element, bpmnFactory, elementRegistry, translate) {
    ...
    approvalDate.getElement(documentGroup, element);
    dateOfDelivery.getElement(documentGroup, element);
    contacts.getElement(documentGroup, element);
    functionalityDomain.getElement(documentGroup, element);
    documentProperty.getElement(documentGroup, element);
...
function createSeventhTabGroups(element, bpmnFactory, elementRegistry, translate) {
    ...
    acceptingConditions.getElement(jobGroup, element);
    tutorial.getElement(jobGroup, element);
    specialist.getElement(jobGroup, element);
    organization.getElement(jobGroup, element);
...
function createDiagramTabGroups(element, bpmnFactory, elementRegistry, translate) {

    var diagramGroup = {
        id: 'diagram',
        label: 'نمودار های مرتبط',
        entries: []
    };

    upDiagram.getElement(diagramGroup, element);
    nextDiagram.getElement(diagramGroup, element);
...
export function getTabs(elem) {
    ...
    var diagramTab = {
        id: 'diagram',
        label: 'نمودار های مرتبط',
        groups: createDiagramTabGroups(elem)
    };
    ...
    if(elem.type == "bpmn:DataObjectReference") {
        ...
        for(var j = 0; j < elements.length; j++) {
            if(elements[j].split('_')[0] == 'StartEvent') {
                startElem = elements[j];
                break;
            }
        }

        if(startElem != null) {
            var outFromStart = cli.element(startElem);
            outFromStart = outFromStart.outgoing;
            for(j = 0; j < outFromStart.length; j++) {
                var objForStart = outFromStart[j].businessObject;
                objForStart = objForStart.targetRef;
                if(objForStart.id == elem.id) {
                    allow2 = true;
                    break;
                }
            }
    ...
    else if(elem.type == "bpmn:Participant") {

        var seventhTab = {
            id: 'job',
            label: 'پروفایل شغل',
            groups: createSeventhTabGroups(elem)
        };
    ...
    if(allow2) {
        var sixthTab = {
            id: 'services2',
            label: 'کاربرگ سند',
            groups: createSixthTabGroups(elem)
        };
