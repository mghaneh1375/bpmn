...
export function ratingBox(options) {
    ...
    resource.html =
        ...
        '>' + label + '</label>' +
        '<div class="bpp-field-wrapper inline_elem" ' +
        '>';

    resource.html +=
        '<input class="defaultInput hidden" type="text" id="' + resource.id + '" ' +
        'name="' + options.modelProperty + '" value="' + options.value + '">' +
        '<div id="importanceRating"></div></div>';
...
export function comboBox(options) {
    ...
    resource.html +=
        ...
        'name="' + options.modelProperty + '" value="' + options.value + '">' +
        '<select style="width: 173px" id="select_' + options.id + '" onchange="changeCombo(this.value, \'' + options.id + '\')"></select></div>';

    resource.typeClass = "combo";
    resource.cssClasses = ['bpp-textbox'];
...
export function textBox(options) {
    ...
    if(options.disable != undefined && (options.disable == "true" || options.disable)) {
        resource.html +=
            '<input disabled readonly type="text" id="' + resource.id + '" ' +
            'name="' + options.modelProperty + '" value="' + options.value + '">' +
            '</div>';
    }
    else {
...
export function dateBox(options) {
    ...
    resource.html +=
        '<input readonly type="text" id="' + resource.id + '" ' +
        'name="' + options.modelProperty + '" value="' + options.value + '">' +
        '<input type="button" style="padding: 10px; margin-right: 10px; border: none; width: 30px; height: 30px; background: url(\'./calendar-flat.png\') repeat 0 0; background-size: 100% 100%;" id="' + resource.id + '_date_btn">';
...
export function file(options) {
    ...
    if(options.value.length > 0) {
        for (var i = 0; i < vals.length; i++) {
            ...
            resource.html += "<div style='margin: 10px' id='file_" + tmp[0] + "'><a target='_blank' href='https://ppl.ut.ac.ir/demo/editor/basedata/downloadFile.php?file_id=" + tmp[0] + "' class='btn btn-default' download>دانلود فایل " + fileName  + "</a><p onclick='removeFile(" + tmp[0] + ", \"" + window.cli.elements()[0] + "\")' class='btn btn-danger' style='padding: 9px; display: inline-block'><span class='glyphicon glyphicon-remove'></span></p></div>";
        }
    }
...
export function textBox2(options) {
    ...
    var tmpLabel = (options.role_value.length > 0) ? "ویرایش" : "انتخاب";

    resource.html =
        '<label for="' + resource.id + '" ' +
        // (canBeShown ? 'data-show="isShown"' : '') +
        '>' + label + '</label><button style="min-width: 80px; margin-right: 10px" class="btn btn-warning" onclick="showPopupSelect(\'' + resource.placeHolder + '\', \'' + resource.id + '\', \'' + options.label + '\')">' + tmpLabel + '</button>' +
        ...
        '<div class="col-xs-12" style="width: 300px" id="' + resource.id + '" />'
    '</div>';
...
export function textBoxWithOutCondition(options) {
    ...
    resource.html =
        '<label for="' + resource.id + '" ' +
        '>' + label + '</label><button style="min-width: 80px; margin-right: 10px" class="btn btn-warning" onclick="showPopupSelect2(\'' + resource.id + '\', \'' + options.label + '\', \'' + options.mode + '\')">' + tmpLabel + '</button>' +
        '<div class="bpp-field-wrapper row" ' +
        '>' +
        '<input class="hidden" value="' + options.role_value + '" data-val="' + options.role_value + '" id="' + resource.id + '"/>' +
        '<div class="col-xs-12" style="width: 300px" id="' + resource.id + '" />' +
    '</div>';
